...
- **Option A (Worker receives money, owes platform):** Customer pays worker in cash. Worker's account accrues "debt" to the platform. _Too complex for a 5-day MVP (requires billing logic and suspensions)._
- **Option B (Platform receives money, pays worker later):** Customer pays via Chapa to the Platform. Platform holds money, pays worker offline on Friday. _Technically this is Escrow/Holding, which the SRS says is unsupported, but it's the easiest way to guarantee commission._
...
5. **Webhook:** Chapa fires a webhook to `POST /api/webhooks/chapa`.
   - _CRITICAL:_ The webhook must verify the Chapa hash signature.
