...
- **Bidding on Jobs:** They view public jobs and submit an `Application`. 
  - *Crucial Requirement:* The application must include a `Proposed Price` and `Estimated Completion Time` (FR-030, FR-031). The worker does not have to accept the customer's stated budget.
