...
_Is this the correct model?_ Yes. Do not put `CUSTOMER` or `WORKER` in the `role` enum on the User table. A human is a `USER`. How they interact with the platform is determined by their active profile.
