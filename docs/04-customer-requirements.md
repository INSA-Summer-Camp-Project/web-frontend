...
- **Direct Hire:** They can visit a Worker's profile and click "Hire Me" to bypass the public job board. *Technical Note:* This still creates a `Job` entity in the database, but pre-assigns the `workerId` and skips the `Application` phase.
- **Application Management:** They compare incoming bids based on price, completion time, and worker rating (FR-033). They select *one* applicant.
...
- They should not be able to edit a Job's budget or scope *after* a worker has been accepted (Architectural constraint to prevent disputes).
